...
public class ColorPixel implements Pixel {
	...
	public Pixel lighten(double factor) {
		...
		double new_Red=getRed()+factor*(1.0-getRed());
		double new_Green=getGreen()+factor*(1.0-getGreen());
		double new_Blue=getBlue()+factor*(1.0-getBlue());
		Pixel pixel=new ColorPixel(new_Red,new_Green,new_Blue); 
		return pixel;
	}
	@Override
	public Pixel darken(double factor) {
		...
		if(factor<0||factor>1)
			throw new RuntimeException("Invalid parameter");
		double new_Red=getRed()-factor*getRed();
		double new_Green=getGreen()-factor*getGreen();
		double new_Blue=getBlue()-factor*getBlue();
		Pixel pixel=new ColorPixel(new_Red,new_Green,new_Blue); 
		return pixel;
	}
}
